...
Track::Track(Ogre::ResourceManager *pManager, const Ogre::String& resource_name,
		...
		Ogre::Resource(pManager, resource_name, handle, resource_group,
				manual_load, pLoader) {
...
void Track::loadImpl() {
	...
	info = Ogre::ResourceGroupManager::getSingleton().findResourceFileInfo(
			mGroup, mName);

	for (Ogre::FileInfoList::const_iterator i = info->begin(); i != info->end();
			++i) {
	...
	if ((_pTrack = Mix_LoadMUS(_path.c_str())) == NULL) {
		pLogManager->logMessage(
				"Track::loadI() Imposible cargar el recurso de sonido.");
		throw(Ogre::Exception(Ogre::Exception::ERR_FILE_NOT_FOUND,
...
void Track::fadeIn(int ms, int loop) {
	...
	if (Mix_FadeInMusic(_pTrack, ms, loop) == -1) {
		pLogManager->logMessage(
				"Track::fadeIn() Error al aplicar efecto de sonido.");
